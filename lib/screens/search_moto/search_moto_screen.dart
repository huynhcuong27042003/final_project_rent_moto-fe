...
import 'package:final_project_rent_moto_fe/services/search_moto/search_moto_service.dart';
...
class _SearchMotoScreenState extends State<SearchMotoScreen> {
  ...
  final _searchMotoService = SearchMotoService();
  ...
  Future<void> _calculateDistances({List<dynamic>? filteredMotorcycles}) async {
    final distanceCalculator = Distance();
    List<dynamic> motorcycleList = filteredMotorcycles ?? await motorcycles;
    ...
    for (var motorcycle in motorcycleList) {
      ...
      try {
        ...
        double distanceInMeters =
            distanceCalculator.as(LengthUnit.Meter, userLocation, motoLocation);
    ...
    setState(() {
      sortedMotorcycles = motorcycleList;
      isLoading = false;
    });
  }

  List<dynamic> _filterMotorcycles(List<dynamic> motorcycleList) {
    return motorcycleList.where((motorcycle) {
      final categoryMatches = selectedCategory == null ||
          motorcycle['category']['name'] == selectedCategory;
      final companyMatches = selectedCompany == null ||
          motorcycle['companyMoto']['name'] == selectedCompany;
  ...
  void _onCategorySelected(String category) {
    setState(() {
      ...
      isLoading = true; // Hiển thị trạng thái tải lại
    });

    motorcycles.then((motorcycleList) {
      final filteredMotorcycles = _filterMotorcycles(motorcycleList);
      _calculateDistances(filteredMotorcycles: filteredMotorcycles);
    });
  }
          ...
          Container(
            ...
            child: Row(
              children: [
                ...
                SizedBox(width: 5),
                _searchMotoService.customIconTextButton(
                  icon: Icons.motorcycle,
                  text: "Loại xe",
                  onPressed: () {
                    _searchMotoService.showBottomCategorySheetModal(
                        context, _onCategorySelected);
                  },
                  context: context,
                ),
                SizedBox(width: 5),
                _searchMotoService.customIconTextButton(
                  ...
                  text: "Hãng xe",
                  onPressed: () {
                    _searchMotoService.showBottomCompanySheetModal(
                        context, _onCompanySelected);
                  },
                  context: context,
                ),
              ],
            ...
            child: FutureBuilder<List<dynamic>>(
              ...
              builder: (context, snapshot) {
                if (snapshot.connectionState == ConnectionState.waiting) {
                  ...
                  if (data.isEmpty) {
                    return const Center(
                        child: Text('Không có xe nào được tìm thấy'));
                  }
